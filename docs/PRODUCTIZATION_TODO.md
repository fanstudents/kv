...
>
> 最後校準：2026-08-02｜最新完成：WP-03 第五段（Orders typed boundary，`159a819`）｜進行中：WP-03｜CodeGraph 於各批驗證後同步
>
...
- [~] 逐 domain 修復不相容 query／mapping；Visit／Visit history、Goals、Checklist、Orders 已完成，其餘 consumer 待分批處理，不改資料格式。
...
- 相容入口刪除條件：既有 Main imports 依 domain 完成 typed migration，`getSupabase`／`LegacyDatabase` production reference 歸零；Orders 批次後尚餘 25 個 reference files，其中 22 個會在 runtime 呼叫、3 個只取用舊 client 的 type。
...
- [~] WP-03 已核准並完成五段：generated types／typed-client seam、Visit／Visit history、Goals、Checklist、Orders；尚餘 22 個 runtime caller files 與 3 個 type-only reference files，後續逐 domain 遷移且每段開始前確認。
...
- 最新完成：`WP-03` 第五段（Orders typed Main DB boundary，`159a819`）；`WP-04` 已完成，WP-03 整包仍在逐 domain 執行。
- 結構基線：已建立，但尚不能宣稱整包架構完成；Contact Research、Visit lock／settings、Visit DB adapters、Goals、Checklist、Orders 已有清楚 owner／typed boundary，其餘 `src/lib`、legacy boundary 與過細 layers 仍待需求／風險驅動收斂。
- 資料庫：Main／Teaching 基線可用；Main generated types 與漸進 typed-client seam 已建立，尚餘 22 個 runtime callers 與 3 個 type-only references。
