...
| Meeting | Blocked | Meeting contract 與 failure-boundary tests 通過；OpenAI、Google read／write provider primitives 通過；hosted `start → command → TTS → STT → finish` 真實 composite 完成，文字回覆與 2 筆 turns 持久化、TTS `audio/mpeg`、STT 可理解、無音訊 finish 為 `recordingSaved=false`；Meeting／turn／usage／storage cleanup residue 為 0；Chrome Meeting 頁正常 | 只缺真人 Chrome camera／mic、Realtime WebRTC 與整場 recording 保存／cleanup；需測試者允許媒體權限並說一小段測試語句 | Meeting session／storage、OpenAI realtime、Chrome media |
