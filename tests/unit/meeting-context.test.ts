...
const { getPipelineOverview, knowledgeContext } = vi.hoisted(() => ({
  getPipelineOverview: vi.fn(),
  knowledgeContext: vi.fn(),
}));
...
vi.mock("@/lib/knowledge-base", () => ({ knowledgeContext }));
...
beforeEach(() => {
  vi.clearAllMocks();
  knowledgeContext.mockResolvedValue("");
});
...
describe("operations meeting context", () => {
  ...
  it("puts an explicit knowledge-base-unavailable instruction in live context", async () => {
    knowledgeContext.mockRejectedValue(new Error("embedding unavailable"));

    await expect(getAgentLiveContext("report", "查詢內部政策")).resolves.toContain(
      "知識庫目前暫時無法檢索",
    );
  });
});
