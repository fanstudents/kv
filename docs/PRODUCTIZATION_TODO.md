...
- [ ] 下一個真實 KB journey 觸碰時，才把 `src/lib/knowledge-base.ts` 的 context、`kb-import.ts`／`kb-search.ts` 的 ingestion/index ownership 往 domain slice touch-and-migrate；不得先做全 repo 搬檔。
...
- `src/lib/**`：共用技術能力與待觸碰 legacy orchestration islands；不是新業務邏輯的預設落點。KB context／ingestion／index 目前仍在這裡，需以真實 journey 觸碰時再收斂。
...
| Knowledge Base／Firecrawl | 真實單頁 journey 已通過；`supabase-knowledge-store.ts` owner `knowledge_base`／`knowledge_access` persistence；`firecrawl-client.ts` owner HTTP／quota／retry，`kb-crawl.ts`／`kb-import.ts` 仍 owner source state／ingestion orchestration | 保持 provider／DB 故障與回滾邊界；下一個真實 KB slice 才收斂 context／index，不建 generic crawler platform |
...
| Knowledge Base | `/knowledge-base/**`、KB APIs／cron | `modules/knowledge-base` + `supabase-knowledge-store` + transitional context／ingestion／Firecrawl composition | Main、Firecrawl、OpenAI | P2／WP-11 |
...
| KB ownership repair | `firecrawl-client.ts` + `kb-crawl.ts` + `supabase-knowledge-store.ts` + focused contracts | Firecrawl protocol／quota／retry、`knowledge_base`／`knowledge_access` persistence 與 source state／ingestion 分責；未新增 route-specific layer；context／index 仍標示 transitional |
