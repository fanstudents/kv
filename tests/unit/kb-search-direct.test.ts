...
describe("kb search and index failure semantics", () => {
  ...
  it("preserves the prior index and reports embedding failure before the atomic replacement", async () => {
    ...
    await expect(indexDocs(["doc-1"])).rejects.toMatchObject({
      name: "KnowledgeIndexError",
      operation: "embedding",
  ...
  it("fails closed without a database lookup when the embedding provider cannot return a query vector", async () => {
    ...
    await expect(searchKnowledge({ question: "方案怎麼買？", maxLevel: 2 })).rejects.toMatchObject({
  ...
  it("reports database failure instead of presenting it as no hits", async () => {
    ...
    await expect(searchKnowledge({ question: "方案怎麼買？", maxLevel: 2, limit: 4 })).rejects.toMatchObject({
      name: "KnowledgeSearchError",
      kind: "database",
