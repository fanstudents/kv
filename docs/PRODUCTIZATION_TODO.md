...
- Last code snapshot：`9375a9a`（Health readiness aligns with server-side writes）。CodeGraph 為 481 files／4,196 nodes／10,542 edges，無 pending drift。
- 本文件 revision 的輸入 snapshot：`9375a9a`；本批補強部署前置判定與回歸證據，不改 UI、API payload、schema 或 provider side effects。
...
| Current verification | `verify:config`、focused contracts、`npm test`、lint/typecheck/build、CodeGraph、Playwright smoke | `verify:config` 通過並只顯示缺少變數名稱；137 files／706 tests、93-page build、147-test hermetic browser smoke；本批完成 Doctor write-readiness 與 `/api/health` 語意對齊，並保留前批 Checklist toggle／rollback、Subscribers tag／broadcast、Meeting start/media boundary 與 aborted-session cleanup 證據；未改 UI／schema／provider side effects；2026-08-15 CodeGraph 481 files／4,196 nodes／10,542 edges，無 pending drift |
...
- [~] 本 branch `npm run verify:full` 已通過 lint、typecheck、137 files／706 tests、93-page production build與 147-test hermetic browser smoke；本批補強 staging／live 需要 service-role 的 Doctor write-readiness guard，並使 `/api/health` 在 anon-only 時回 503/degraded；保留 Checklist／Subscribers functional browser contracts、Meeting start／media boundary contracts與 aborted-session cleanup，未改 UI／schema。另以 `npm run test:e2e:run:staging` 對真實 Main read paths 跑同一批 136 tests，無 assertion failure；147-test full smoke 的缺 Supabase 日誌來自未攔截的無憑證 read surfaces，並非測試失敗。locked install、hosted artifacts／flaky 分類仍待 canonical repo。
...
- [~] 全量 verify、CodeGraph、137-file／706-test contracts、8-page Chrome matrix、Main residue audit 與 147-test browser smoke 已完成；本批未改 UI，除既有 Goals／Agent settings／Checklist／Subscribers／Meeting 邊界證據外，新增 staging／live anon-only 會被 Doctor 擋下、`/api/health` 回 degraded 的設定契約；staging cutover／rollback rehearsal 仍待 deploy ownership。
...
- [x] focused contract 已覆蓋設定輸入、pending invite、Gmail provider 呼叫、LINE 回覆、run telemetry 與 conversation lock release；目前完整 verify 已通過 137 files／706 tests、93-page build、147-test hermetic browser smoke。
