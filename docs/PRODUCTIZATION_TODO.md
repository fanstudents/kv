...
| Current verification | `verify:config`、focused contracts、`npm test`、lint/typecheck/build、CodeGraph、Playwright smoke | `verify:config` 通過並只顯示缺少變數名稱；138 files／717 tests、93-page build、147-test hermetic browser smoke；release preflight 已在 evidence baseline `47e9caf` 成功產生 6-migration manifest，migration plan 在缺 `SUPABASE_DB_URL` 時安全停止；本機無 Docker／Podman，因此本批 local schema replay deferred，既有 hosted CI schema job仍是 canonical replay gate；未改 UI／schema／provider side effects；2026-08-15 CodeGraph 483 files／4,222 nodes／10,620 edges，無 pending drift |
