...
export default function TrafficOverviewPanel() {
  ...
  useEffect(() => {
    let alive = true;
    fetch("/api/agents/report/traffic-overview")
      .then((r) => r.json())
      .then((d) => {
        if (!alive) return;
  ...
  return (
    <Card className="mb-6">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-neutral-700 dark:text-neutral-200">GA4 真實流量(近 7 天)</h2>
      ...
      <p className="mb-2 text-xs font-medium text-neutral-500">每日工作階段趨勢(近 14 天)</p>
