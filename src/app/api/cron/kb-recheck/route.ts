...
export async function GET(req: NextRequest) {
  ...
  const auth = parseKnowledgeCrawlRecheckAuth(process.env.CRON_SECRET, req.headers.get("x-cron-key"));
  if (auth.kind !== "authorized") return NextResponse.json({ error: auth.message }, { status: auth.status });
  return NextResponse.json(await recheckKnowledgeSources(createFirecrawlKnowledgeSource()));
}
