...
| Current verification | `verify:config`、focused contracts、`npm test`、lint/typecheck/build、CodeGraph、Playwright smoke | `verify:config` 通過並只顯示缺少變數名稱；138 files／717 tests、93-page build、147-test hermetic browser smoke；Main staging Chromium read／functional matrix 147/147；release preflight 通過，migration plan 以 Session Pooler 連線後六筆 history 全部一致且 dry-run `upToDate`；缺 `SUPABASE_DB_URL` 時安全停止仍有測試證據；本機無 Docker／Podman，因此本批 local schema replay deferred，既有 hosted CI schema job仍是 canonical replay gate；未改 UI／schema／provider side effects；2026-08-15 CodeGraph 483 files／4,222 nodes／10,620 edges，無 pending drift |
...
- [~] 本 branch `npm run verify` 已通過 lint、typecheck、138 files／717 tests 與 93-page production build；evidence baseline `47e9caf` 的 147-test hermetic browser smoke 全數通過，現行 staging matrix 在 `f61a82f` 後 147/147 通過。release preflight／migration plan 亦通過；locked install、hosted artifacts／flaky 分類仍待 canonical repo。
...
- [x] `release:migrations:plan` 先做 migration history compare 與 `db push --dry-run`；`release:migrations:apply` 要求 DB URL 命中 allowlisted project ref、backup 已確認、explicit apply gate 與相同 project confirmation。沒有 `SUPABASE_DB_URL` 時安全停止已被證明；本批以重設後的 Session Pooler URL 完成 history compare／dry-run，結果為六筆一致、remote `upToDate`，未套用 migration。
...
- [!] 外部 release gate 尚未封口：DB URL 已在 Git ignored `.env.local` 設定，換機時需由 secret store 安全重建；仍需 canonical GitHub／Zeabur staging owner、部署 exact commit、`release:verify` 與 application rollback rehearsal；staging DB migration 本身已驗證為 up-to-date。
...
0. **P0 — Scope control（D，不阻塞 P1／P2／P4／W1）**
   ...
   - [x] 以上功能沿現有 domain owner 交付：① Visit／企業拜訪（名片→研究→邀約→Calendar／Gmail／LINE）；② Teachify／課程或電商訂單通知（webhook→Orders→Primary LINE）；③ Knowledge／客服內訓（知識庫→審核／搜尋→Support Bot）；其餘既有頁面與能力維持 UI／API 相容並納入同一 release checklist。
